...
public partial class Student_CourseQuizExam : ClsPageEvents, IPageInterFace
{
    ...
    {
        DS_RECORD = objExam.FnGetCourseExamExamQuizRecords(FnIsNumeric(FnDecryptQueryString(Request.QueryString["CRSEID"].ToString())));
    ...
    protected void RptrAssignment_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                (item.FindControl("HyLnkView") as HyperLink).NavigateUrl = FnRedirectNextPage("CourseQuizExamList.aspx", "", "&CRSEID=" + Request.QueryString["CRSEID"].ToString() + "&INDX=5&ASGID=" + FnEncryptQueryString(dr["Id"].ToString().ToString()));
            }
        }
        catch (Exception ex)
        {
